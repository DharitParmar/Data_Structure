...
void input_validation(char choice)
{
	...
	do
	{
		if (choice == 'x' || choice == 'X') {
			...
			while (!cin)
			{
				cout << "This isn't valid format! Please enter an integer: " << endl;
				cin.clear();
				cin.ignore();
...
void arrival(int num)
{
	...
	else if (!(g2.full_q()))
	{
		...
		g2.insert_q(num);
		cout << "Your vehicle " << num << " parked safly in 'Bashemup'." << endl;
	}
	else if (!(street.full_q()))
	{
		street.insert_q(num);
		cout << "Your vehicle " << num << " parked safly on street." << endl;
	}
	else
	{
